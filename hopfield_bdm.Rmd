...
To compute algorithmic complexity on magnetization data we first had to bin the data. We figured that there are two reasonable ways of doing it: compute quantiles or just divide the line section from 0 to 1 into even number of bins.
...
bdm = BDM(ndim=1, nsymbols=9)
data.seq = data.seq.apply(lambda x: np.array(x, dtype = int) - 1)

mags_quantiles = pd.DataFrame({"name": data.name,
                               "ncmx": data.seq.apply(bdm.nbdm),
                               "cmx": data.seq.apply(bdm.bdm)})
...
data = r.mags_even_bins
bdm = BDM(ndim=1, nsymbols=9)
data.seq = data.seq.apply(lambda x: np.array(x, dtype = int) - 1)

even_bins = pd.DataFrame({"name": data.name,
                          "ncmx": data.seq.apply(bdm.nbdm),
                          "cmx": data.seq.apply(bdm.bdm)})
```

### The difference between quantiles and even bins

```{r difference_between_bins}
mags_quantiles_cmx <- py$mags_quantiles %>%
  mutate(bins = "quantiles") %>%
  unnest()
mags_even_bins_cmx <- py$even_bins %>%
  mutate(bins = "even_bins") %>%
  unnest()

mags_quantiles_cmx %>%
  bind_rows(mags_even_bins_cmx) %>%
  gather(key = "klucz",
         value = "wartosc",
         cmx:ncmx) %>%
